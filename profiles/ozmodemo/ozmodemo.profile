...
define('DEMO_PROFILE_NAME', 'ozmodemo');
define('DEMO_PROFILE_DESCRIPTION', 'Custom ozmosis profile with demo data');
define('DEMO_PROFILE_DUMP', '/ozmo.sql');
define('DEMO_DUMP', '/demo.sql');
define('DEMO_HOME', '/demo\.tenet\.ozmosis\.com/');

function ozmodemo_profile_modules() {
    return array();
}

function ozmodemo_profile_details() {
    return array(
...
function ozmodemo_profile_task_list() {
}

function ozmodemo_profile_tasks(&$task, $url) {
    $this_dir = dirname(__FILE__);

// Import a dump file
    $dump_file = $this_dir . DEMO_PROFILE_DUMP;
    $success = import_demo_dump($dump_file, FALSE);
    if (!$success) {
        return;
    }

    $dump_file = $this_dir . DEMO_DUMP;
    $success = import_demo_dump($dump_file, TRUE);
    if (!$success) {
        return;
    }
...
function ozmodemo_form_alter(&$form, $form_state, $form_id) {
    if ($form_id == 'install_configure') {
        $form['#submit'][] = 'ozmodemo_form_submit';
    }
}

function ozmodemo_form_submit($form, &$form_state) {
    $dump_file = dirname(__FILE__) . DEMO_PROFILE_DUMP;
    $success = import_demo_dump($dump_file, FALSE);
    if (!$success) {
        return;
    }
...
function import_demo_dump($filename, $demo=FALSE) {
    ...
    if (!$demo){
        foreach (list_all_tables() as $table) {
    ...
    while (!feof($fp)) {
        ...
        $data = preg_replace(DEMO_HOME, $my_uri, $data);
